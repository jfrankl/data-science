...
def in_winnebago_county(case):
    if case["result"]["countyName"] == "Winnebago":
        return True
    else:
        return False
...
with open("final/cases-all.ndjson") as c:
    ...
    filtered = filter(in_winnebago_county, cases)

    case_nums = list(map(get_case_nums, filtered))

    with open("test/winnebago-county.csv", newline="") as file:
